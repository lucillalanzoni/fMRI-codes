...
for i in range(len(par)):
    ...
    for k in range(len(behpath)):
        ...
        fout1 = open('/scratch/groups/Projects/P1334/1stLevel/%s/Evs/task_singleEVs_collapsed/Run%s_combined_task.txt' %(par[i], run),'w')
        fout2 = open('/scratch/groups/Projects/P1334/1stLevel/%s/Evs/task_singleEVs_collapsed/Run%s_scrambled_task.txt' %(par[i],run),'w')        
        fout3 = open('/scratch/groups/Projects/P1334/1stLevel/%s/Evs/task_singleEVs_collapsed/Run%s_single_task.txt' %(par[i],run),'w')
        fout4 = open('/scratch/groups/Projects/P1334/1stLevel/%s/Evs/task_singleEVs_collapsed/Run%s_nonSem_task.txt' %(par[i],run),'w')
        
        fout5 = open('/scratch/groups/Projects/P1334/1stLevel/%s/Evs/cue_singleEVs_collapsed/Run%s_combined_pics.txt' %(par[i], run),'w')
        fout6 = open('/scratch/groups/Projects/P1334/1stLevel/%s/Evs/cue_singleEVs_collapsed/Run%s_scrambled_pics.txt' %(par[i],run),'w')
        fout7 = open('/scratch/groups/Projects/P1334/1stLevel/%s/Evs/cue_singleEVs_collapsed/Run%s_single_pics.txt' %(par[i],run),'w')
        fout8 = open('/scratch/groups/Projects/P1334/1stLevel/%s/Evs/cue_singleEVs_collapsed/Run%s_nonSem_pics.txt' %(par[i],run),'w')
        ...
        for entry in trials:
            ...
            if entry['Cue Condition'] == 'combined':
                ...
                output = print('{0} {1} {2}'.format(entry['Cue_start'],'1','1'), file = fout5)
            ...
            if entry['Cue Condition'] == 'singleE'or entry['Cue Condition'] == 'singleL': 
